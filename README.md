...
One benefit of such technology is the ability to render extensive heightmap terrains without need for (dynamic) mesh construction and upload to the GPU as seen in modern, triangle-based voxel ga.g.mes (e Minecraft), each time the terrain mesh changes (which can happen often). This speeds up dev times for indie developers and small studios who only want to get a feel for a dynamic 3D terrain without extensive effort.
